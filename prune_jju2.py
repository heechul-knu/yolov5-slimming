...
#    print(total)
    ...
    for k, m in enumerate(model.modules()):
        ...
        if isinstance(m, nn.Sequential): # 두번째 sequential에서부터 counting
            N += 1
...
#          cnt += 1
    ...
    old_modules = list(model.named_parameters())
    new_modules = list(newmodel.named_parameters())
    mlist = list(model.modules())
    layer_id_in_cfg = 0
    ...
    for layer_id in range(len(old_modules)):
        ...
        if 'model.1' in name0 or 'model.3' in name0 or 'model.5' in name0 \
            or 'model.7' in name0 or 'model.10' in name0 or 'model.14' in name0 \
                or 'model.18' in name0 or 'model.21' in name0 \
                    or 'model.2.cv' in name0 or 'model.4.cv' in name0 or 'model.6.cv' in name0 \
                        or 'model.9.cv' in name0 or 'model.13.cv' in name0 or 'model.17.cv' in name0 \
                            or 'model.20.cv' in name0 or 'model.23.cv' in name0:

            if 'bn.weight' in name0:
                print("batchnorm!!!!!!!!!!!!!!!!")
                idx1 = np.squeeze(np.argwhere(np.asarray(end_mask.cpu().numpy())))
                if idx1.size == 1:
                    idx1 = np.resize(idx1,(1,))
                #print(idx1.size)
                m1.data = m0.data[idx1.tolist()].clone()
                m1.running_mean = m0.running_mean[idx1.tolist()].clone()
                m1.running_var = m0.running_var[idx1.tolist()].clone()
            elif 'bn.bias' in name0:
                print("batchnorm!!!!!!!!!!!!!!!!")
                idx1 = np.squeeze(np.argwhere(np.asarray(end_mask.cpu().numpy())))
                if idx1.size == 1:
                    idx1 = np.resize(idx1,(1,))
                #print(idx1.size)
                m1.weight.data = m0.data[idx1.tolist()].clone()
                m1.bias.data = m0.data[idx1.tolist()].clone()
                m1.running_mean = m0.running_mean[idx1.tolist()].clone()
                m1.running_var = m0.running_var[idx1.tolist()].clone()
            elif isinstance(name0, nn.Conv2d):
                print("conv2d!!!!!!!!!!!!!!!!!")
